...
class ESP32GPIO:
    ...
    def is_connected(self) -> bool:
        ...
        return self.ser is not None and self._is_running
    ...
    def i2c_init(self, sda: int, scl: int) -> None:
        """
        Initialize I2C bus.
        ...
        """
        self._send_command(f"I2C_INIT {sda} {scl}")
    ...
    def i2c_scan(self) -> List[str]:
        ...
        Returns:
            List of I2C device addresses as hex strings
        """
    ...
    def i2c_write(self, addr: str, data_bytes: List[int]) -> None:
        """
        Write data to I2C device.

        Args:
            addr: I2C device address as hex string (e.g., '0x68')
            data_bytes: List of data bytes to write
        """
        hex_data = " ".join([f"0x{b:02x}" for b in data_bytes])
        self._send_command(f"I2C_WRITE {addr} {hex_data}")

    def i2c_read(self, addr: str, num_bytes: int) -> List[int]:
        """
        ...
        Returns:
            List of read bytes
        """
        response = self._send_command(f"I2C_READ {addr} {num_bytes}")
        if not response:
            return []
        return [int(b, 16) for b in response.split()]

    def i2s_init_tx(self, bck_pin: int, ws_pin: int, data_pin: int, sample_rate: int = 44100) -> None:
        """
        Initialize I2S transmitter.

        Args:
            bck_pin: BCK (bit clock) pin number
            ws_pin: WS (word select) pin number
            data_pin: Data output pin number
            sample_rate: Sample rate in Hz (default: 44100)
        """
        self._send_command(f"I2S_INIT_TX {bck_pin} {ws_pin} {data_pin} {sample_rate}")
        self.i2s_initialized = True

    def i2s_write(self, data: List[int]) -> None:
        """
        Write audio data to I2S bus.

        Args:
            data: List of 16-bit signed integer audio samples
        """
        data_str = " ".join(map(str, data))
        self._send_command(f"I2S_WRITE {data_str}")
    ...
    def reset_pin(self, pin: int) -> None:
        """
        Reset a pin to input mode (removes from configured pins).

        Args:
            pin: GPIO pin number to reset
        """
        if pin in self.configured_pins:
            self.configured_pins.remove(pin)
        # Note: ESP32 firmware doesn't have a reset command, so we just track locally
